...
function f16() {
    ...
    let outName = ''
    for (let key in a16) {
        outName += a16[key]['name'] + ' ';
    }
    out16.innerHTML = outName;
}
...
function f17() {
    ...
    for (let key in a17) {
        if (a17[key]['age'] > 30) {
            outName += a17[key]['name'] + ' ';
        }
    }
    out17.innerHTML = outName
}
...
function f18() {
    ...
    if (a18[input] !== undefined) {
        for (let i = 0; i < a18[input].length; i++) {
            outStation += a18[input][i] + ' ';
        }
    }
    out18.innerHTML = outStation;
...
function f19() {
    ...
    let inputStation = document.querySelector('.i-19').value.toLowerCase();
    let outStation = '';

    for (let branch in a19) {
        for (let i = 0; i < a19[branch].length; i++) {
            if (a19[branch][i].toLowerCase() == inputStation) {
                outStation = branch;
            }
        }
    }
    out19.innerHTML = outStation;
}
