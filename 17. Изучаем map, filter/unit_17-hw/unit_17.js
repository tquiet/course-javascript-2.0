...
function t9() {
    ...
    a9.forEach(function (item) {
        a9_res.push(item.toLocaleLowerCase())
    })
    a9 = a9_res
    return a9
...
function t14() {
    let a14_res = [];
    let sym = a14_sym.toLocaleLowerCase();

    a14.forEach(function (item) {
        a14_res.push(item.toLocaleLowerCase());
    })
    return a14_res.includes(sym)
}
